...
async function getApp() {
  if (!appPromise) {
    appPromise = (async () => {
      ...
      // 🔥 ROTA ADICIONAR PRODUTO (ATUALIZADA COM BADGE E COR)
      ...
      // 🔥 ROTA EDITAR PRODUTO (ATUALIZADA COM BADGE E COR)
      app.post('/admin/products/edit/:id', isAdmin, async (req, res) => {
        try {
          // 🔥 Adicionamos badge_color ao destructuring
          const { name, description, price, category, thumbnail, video_link, features, badge, badge_color } = req.body;

          // 🔥 Incluímos badge_color na query e nos valores
          await pool.query(
